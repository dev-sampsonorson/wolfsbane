...
export default class TeboUtility {
    ...
    private static addEvent(el, type, handler) {
        if (el.attachEvent) el.attachEvent('on' + type, handler); else el.addEventListener(type, handler);
    }

    public static live(selector, event, callback, context) {
    ...
    public static hasExtension(fileName, extensions: string[]) {
        return (new RegExp('(' + extensions.join('|').replace(/\./g, '\\.') + ')$', 'i')).test(fileName);
    }

    public static generateUniqueNumber(seed: number): number {
        return Date.now() + seed;
    }

    public static generateUniqueString(seed: number = 0): string {
        return uuidv4() + (seed ? (':' + seed) : '');
    }
}