import IDownloadableBeatsConfig from "./IDownloadableBeatsConfig";
...
export default class DownloadableBeats {
    ...
    private downloadEls: HTMLElement[];

    private mediaItemTemplate: string;
    private mediaList: IDownloadableMediaItem[];

    private equalizerEl: HTMLElement | null;
    private animatedEqualizer: AnimatedEqualizer | null;

    constructor(private config: IDownloadableBeatsConfig) {
        this.audioEls = [];
        this.durationEls = [];
        this.lineNumberEls = [];
        this.artistEls = [];
        ...
        this.mediaItemTemplate = this.config.mediaItemTemplate;
        this.mediaList = this.config.mediaList;
        ...
        this.config.mediaList.forEach(_ => this.targetEl.innerHTML += this.mediaItemTemplate);
    ...
    private prepareMediaSources(elements: HTMLElement[]) {
        for (let i = 0; i < elements.length; i++) {
            if (this.mediaList[i] === undefined)
                return;

            let itemEl: HTMLElement = this.itemEls[i];
            let theAudio: HTMLMediaElement = document.createElement('audio');
            let mediaUrlMp3: string | null = this.mediaList[i].mp3Url; // itemEl.getAttribute(this.config.dataAttrMediaUrlMp3Key)!;
            let mediaUrlOgg: string | null = this.mediaList[i].oggUrl; // itemEl.getAttribute(this.config.dataAttrMediaUrlOggKey)!;
            let mediaUrlFallback: string | null = this.mediaList[i].fallbackUrl; // itemEl.getAttribute(this.config.dataAttrMediaUrlFallbackKey)!;
            ...
            this.downloadEls[i] = itemEl.querySelector(`:scope .${this.config.downloadElClass}`)! as HTMLElement;

            // console.log('this.lineNumberEls[i]', this.lineNumberEls[i]);

            this.lineNumberEls[i].innerHTML = this.mediaList[i].linenumber;

            this.artistEls[i].innerHTML = this.mediaList[i].artist;
            this.artistEls[i].setAttribute('title', this.mediaList[i].artist);

            this.titleEls[i].innerHTML = this.mediaList[i].title;
            this.titleEls[i].setAttribute('title', this.mediaList[i].title);

            this.downloadEls[i].setAttribute('href', this.mediaList[i].mp3Url);
